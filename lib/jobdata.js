...
var debug = require('debug')('strider-simple-runner:jobdata');
...
JobData.prototype = {
  ...
  pop: function (id) {
    ...
    if (!job) {
      debug('tried to pop a nonexistant job', id);
      return;
    }
    delete this.jobs[id];
    return job;
  ...
  listeners: {
    ...
    'command.comment': function (data) {
      ...
      var command = _.extend({}, consts.skels.command);
    ...
    'command.start': function (data) {
      var phase = this.phases[this.phase];
      var command = _.extend({}, consts.skels.command, data);
      phase.commands.push(command);
    },
    ...
    'plugin-data': function (data) {
      ...
      var parent = path.reduce(function (obj, attr) {
      ...
      if (method === 'replace') {
        ...
        debug('Invalid "plugin data" method received from plugin', data.plugin, data.method, data);
      }
    }
  }
};
